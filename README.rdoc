...
:login:: A #call able ruby object to recieve a username and password and return
a set of ElasticSearch filters.

=== Example config
  # This method must return an array of two elements, the user_id to identify
the access control level this user (or group of users) are assigned and a set
of filters. The same user_id should be returned for the same filters assigned.
  def login(user, pass)
